...
(function(window) {
  ...
  /* ── Demo data for non-doctor categories ── */
  var DEMO_ENTITIES = {
    hospitals: [
  ...
  var DataLayer = {
      ...
      return new Promise(function(resolve, reject) {
        ...
        for (var ci = 0; ci < cats.length; ci++) {
          ...
          for (var ei = 0; ei < cat.length; ei++) {
            if (cat[ei].id === id || cat[ei].slug === id) { resolve(cat[ei]); return; }
          }
        }
    ...
    _demoGetDoctors: function(filters) {
      return new Promise(function(resolve) {
        ...
        if (filters.q) {
          var q = filters.q.toLowerCase();
          results = results.filter(function(d) { return d.name.toLowerCase().indexOf(q)!==-1||(d.subtitle&&d.subtitle.toLowerCase().indexOf(q)!==-1); });
        }
        if (filters.specialty) results = results.filter(function(d) { return d.specialty===filters.specialty||(d.tags&&d.tags.indexOf(filters.specialty)!==-1); });
        if (filters.gender) results = results.filter(function(d) { return d.name.indexOf(filters.gender)!==-1||d.gender===filters.gender; });
    ...
    _demoGetDoctor: function(id) {
      return new Promise(function(resolve, reject) {
        ...
        for (var i=0; i<DEMO_DOCTORS.length; i++) {
          ...
          if (d.id===sid || d.slug===sid || d.name===sid) { found=d; break; }
        }
        if (!found) {
          for (var j=0; j<DEMO_DOCTORS.length; j++) {
            if (DEMO_DOCTORS[j].slug && DEMO_DOCTORS[j].slug.indexOf(sid)!==-1) { found=DEMO_DOCTORS[j]; break; }
          }
        }
        if (found) resolve(found); else reject(new Error('Doctor not found: '+id));
      });
    },
    _supabaseHeaders: function() { return {'apikey':CONFIG.supabase.anonKey,'Authorization':'Bearer '+CONFIG.supabase.anonKey,'Content-Type':'application/json','Prefer':'count=exact'}; },
    _supabaseGetDoctors: function(filters) {
      var url=CONFIG.supabase.url+'/rest/v1/'+CONFIG.supabase.tableDoctors, params=[];
      if (filters.q) params.push('or=(name.ilike.*'+encodeURIComponent(filters.q)+'*,subtitle.ilike.*'+encodeURIComponent(filters.q)+'*)');
      if (filters.specialty) params.push('specialty=eq.'+encodeURIComponent(filters.specialty));
      if (filters.gender) params.push('gender=eq.'+encodeURIComponent(filters.gender));
      ...
      return fetch(url,{headers:this._supabaseHeaders()}).then(function(res){
        ...
        return res.json().then(function(data){ return {doctors:data,total:total,page:page,totalPages:Math.ceil(total/CONFIG.perPage)}; });
      });
    },
    _supabaseGetDoctor: function(id) {
      return fetch(CONFIG.supabase.url+'/rest/v1/'+CONFIG.supabase.tableDoctors+'?id=eq.'+encodeURIComponent(id),{headers:this._supabaseHeaders()})
        .then(function(r){return r.json();}).then(function(d){ if(d&&d.length) return d[0]; throw new Error('not found'); });
    },
    _apiGetDoctors: function(filters) {
      var url=CONFIG.api.baseUrl+CONFIG.api.doctorsEndpoint, p=new URLSearchParams();
      if(filters.q) p.set('q',filters.q); if(filters.specialty) p.set('specialty',filters.specialty);
      if(filters.gender) p.set('gender',filters.gender); p.set('page',filters.page||1); p.set('limit',CONFIG.perPage);
      return fetch(url+'?'+p.toString()).then(function(r){return r.json();}).then(function(data){
        return {doctors:data.data||data.doctors||data, total:data.total||0, page:data.page||filters.page||1, totalPages:data.totalPages||Math.ceil((data.total||0)/CONFIG.perPage)};
      });
    },
    _apiGetDoctor: function(id) { return fetch(CONFIG.api.baseUrl+CONFIG.api.doctorsEndpoint+'/'+encodeURIComponent(id)).then(function(r){return r.json();}); }
  };
  ...
  function initMobileMenu() {
    var btn=document.getElementById('smgMenuBtn'), nav=document.getElementById('smgNav'), overlay=document.getElementById('smgMenuOverlay');
    if(!btn||!nav) return;
    function toggle(){ nav.classList.toggle('open'); if(overlay) overlay.classList.toggle('open'); var o=nav.classList.contains('open'); btn.querySelector('.material-icons-round').textContent=o?'close':'menu'; btn.setAttribute('aria-expanded',o); }
    function close(){ nav.classList.remove('open'); if(overlay) overlay.classList.remove('open'); btn.querySelector('.material-icons-round').textContent='menu'; btn.setAttribute('aria-expanded','false'); }
    btn.addEventListener('click',toggle); if(overlay) overlay.addEventListener('click',close);
  ...
  /* ── Populate hero search dropdown (doctors + other categories) ── */
  SMG.populateSearchDropdown = function(selectId) {
    var sel = document.getElementById(selectId);
    if (!sel) return;
    // Doctor specialties group
  ...
  /* ── Populate specialty-only dropdown (for doctors listing sidebar) ── */
  SMG.populateSpecialtyDropdown = function(selectId) {
    var sel=document.getElementById(selectId); if(!sel) return;
    SPECIALTIES.forEach(function(s){ var o=document.createElement('option'); o.value=s.slug; o.textContent=s.name; sel.appendChild(o); });
  };

  /* ── Search route handler (used on homepage) ── */
  SMG.handleHeroSearch = function(q, val) {
    if (!val) {
      // No type selected — search all doctors by name
    ...
    if (val.indexOf('c:') === 0) {
      ...
      // Doctor specialty (s: prefix or raw specialty slug)
  ...
  SMG.renderDoctorsGrid = function(gridId, paginationId, filters) {
    ...
    DataLayer.getDoctors(filters).then(function(result) {
      if(!result.doctors||result.doctors.length===0){ gridEl.innerHTML='<p class="no-results">لا توجد نتائج. جرّب البحث بكلمات مختلفة.</p>'; if(pagEl) pagEl.innerHTML=''; return; }
      var html='';
      result.doctors.forEach(function(doc) {
        var imgHtml=doc.image?'<img alt="'+escHtml(doc.name)+'" class="smg-card-img" src="'+escHtml(doc.image)+'" loading="lazy">'
      ...
      if(pagEl&&result.totalPages>1){
        ...
        for(var p=1;p<=tp;p++) phtml+=p===cp?'<span class="current">'+p+'</span>':'<a onclick="SMG.goToPage('+p+')">'+p+'</a>';
    ...
    }).catch(function(err){ gridEl.innerHTML='<p class="no-results">حدث خطأ أثناء تحميل البيانات.</p>'; console.error('[SMG]',err); });
  };

  SMG.goToPage = function(page) { var p=new URLSearchParams(window.location.search); p.set('page',page); window.location.search=p.toString(); };

  /* ── Render category entity cards grid ── */
    ...
    DataLayer.getEntities(category, filters).then(function(result) {
      ...
      result.items.forEach(function(ent) {
        var imgHtml = ent.image
          ...
          : '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:' + escHtml(meta.bg) + ';color:' + escHtml(meta.color) + '">'
            + '<span class="material-icons-round" style="font-size:52px">' + escHtml(meta.icon) + '</span></div>';
      ...
      if (pagEl && result.totalPages > 1) {
        var phtml = '', cp = result.page, tp = result.totalPages;
        if (cp > 1) phtml += '<a onclick="SMG.goToPage(' + (cp - 1) + ')" title="السابق"><span class="material-icons-round">chevron_right</span></a>';
        for (var pg = 1; pg <= tp; pg++) phtml += pg === cp ? '<span class="current">' + pg + '</span>' : '<a onclick="SMG.goToPage(' + pg + ')">' + pg + '</a>';
        if (cp < tp) phtml += '<a onclick="SMG.goToPage(' + (cp + 1) + ')" title="التالي"><span class="material-icons-round">chevron_left</span></a>';
        pagEl.innerHTML = phtml;
      } else if (pagEl) pagEl.innerHTML = '';
    }).catch(function(err) {
  ...
  /* ── Render entity (non-doctor) detail profile ── */
  SMG.renderEntityProfile = function(id) {
    DataLayer.getEntity(id).then(function(ent) {
      ...
      if (bCatEl) { bCatEl.textContent = meta.listName; bCatEl.href = './listing.html?category=' + encodeURIComponent(ent.category); }
      ...
      if (avatarEl) {
        if (ent.image) {
          ...
          avatarEl.innerHTML = '<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:' + escHtml(meta.bg) + ';border-radius:50%">'
            + '<span class="material-icons-round" style="font-size:3rem;color:' + escHtml(meta.color) + '">' + escHtml(meta.icon) + '</span></div>';
        }
      }

      var nameEl = document.getElementById('entityName'); if (nameEl) nameEl.textContent = ent.name;
      var subEl = document.getElementById('entitySubtitle'); if (subEl) subEl.textContent = ent.subtitle || '';
      ...
      if (tagsEl) {
        ...
        arr.forEach(function(t) { var sp = document.createElement('span'); sp.textContent = t; tagsEl.appendChild(sp); });
      }
      ...
      var addrEl = document.getElementById('addressText'); if (addrEl) addrEl.textContent = ent.address || 'العنوان غير متوفر';
      var svcEl = document.getElementById('entityServices'); if (svcEl) svcEl.textContent = ent.services || ent.subtitle || meta.name;
      var hoursEl = document.getElementById('entityHours'); if (hoursEl && ent.hours) hoursEl.textContent = ent.hours;
      ...
      if (copyBtn) {
        copyBtn.addEventListener('click', function() {
          ...
          else { var ta = document.createElement('textarea'); ta.value = window.location.href; document.body.appendChild(ta); ta.select(); document.execCommand('copy'); document.body.removeChild(ta); }
          btn.innerHTML = '<span class="material-icons-round">check</span> تم النسخ';
          setTimeout(function() { btn.innerHTML = '<span class="material-icons-round">content_copy</span> نسخ الرابط'; }, 1500);
        });
      }

      // Share btn
      var shareBtn = document.getElementById('shareBtn'), shareFallback = document.getElementById('shareFallback');
      if (shareBtn) {
        var pUrl = window.location.href, pTitle = ent.name + ' — دليل شبين الكوم الطبي';
        var wa = document.getElementById('shareWA'), fb = document.getElementById('shareFB'), msg = document.getElementById('shareMsg');
      ...
      if (schemaEl) {
        ...
        var s = {
          ...
          'address': { '@type': 'PostalAddress', 'addressLocality': 'شبين الكوم', 'addressRegion': 'المنوفية', 'addressCountry': 'EG', 'streetAddress': ent.address || '' },
          'telephone': (ent.phones && ent.phones[0]) || '',
          'url': window.location.href,
          'parentOrganization': { '@type': 'Organization', 'name': 'دليل شبين الكوم الطبي', 'url': window.location.origin }
        };
        if (ent.image) s.image = ent.image;
        schemaEl.textContent = JSON.stringify(s);
      }
    }).catch(function(err) {
      var n = document.getElementById('entityName'); if (n) n.textContent = 'المنشأة غير موجودة';
      var b = document.getElementById('breadcrumbName'); if (b) b.textContent = 'غير موجود';
      console.error('[SMG]', err);
    });
  };
    ...
    DataLayer.getDoctor(id).then(function(doc) {
      ...
      if(avatarEl) {
        if(doc.image) {
          ...
          var init = doc.name ? (doc.name.indexOf('.')!==-1 ? doc.name[doc.name.indexOf('.')+2] : doc.name[0]) : 'د';
      ...
      var nameEl=document.getElementById('doctorName'); if(nameEl) nameEl.textContent=doc.name;
      var subEl=document.getElementById('doctorSubtitle'); if(subEl) subEl.textContent=doc.subtitle||'';
      ...
      var addrEl=document.getElementById('addressText'); if(addrEl) addrEl.textContent=doc.address||'العنوان غير متوفر';
      var svcEl=document.getElementById('doctorServices'); if(svcEl) svcEl.textContent=doc.services||doc.subtitle||doc.specialty||'';
      var hoursEl=document.getElementById('doctorHours'); if(hoursEl&&doc.hours) hoursEl.textContent=doc.hours;
      ...
      var schemaEl=document.getElementById('schemaOrg');
      if(schemaEl) {
        var s={'@context':'https://schema.org','@type':'Physician','name':doc.name,'description':doc.subtitle||'','medicalSpecialty':doc.specialty||'','address':{'@type':'PostalAddress','addressLocality':'شبين الكوم','addressRegion':'المنوفية','addressCountry':'EG','streetAddress':doc.address||''},'telephone':(doc.phones&&doc.phones[0])||'','url':window.location.href,'parentOrganization':{'@type':'Organization','name':'دليل شبين الكوم الطبي','url':window.location.origin}};
        if(doc.image) s.image=doc.image;
        schemaEl.textContent=JSON.stringify(s);
      }
    }).catch(function(err){
      var n=document.getElementById('doctorName'); if(n) n.textContent='الطبيب غير موجود';
      var b=document.getElementById('breadcrumbName'); if(b) b.textContent='غير موجود';
      console.error('[SMG]',err);
    });
  };
